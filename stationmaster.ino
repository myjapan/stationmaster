...
Adafruit_PWMServoDriver* bls[] = {&led2d, &led1d, &led1s, &led1s, &led1s, &led1s, &led1s, &led1s, &led1s};
Adafruit_PWMServoDriver* bld[] = {&led1s, &led2s, &led1d, &led1d, &led1d, &led1d, &led1d, &led1d, &led1d};
...
void setup()
{
  ...
  for(byte i = 0; i < button_total; i++)
  {
    ...
    bp[i]->setPWM(point_id[i], 0, p_direction[i]);
    delay(200);
    bp[i]->setPWM(point_id[i], 0, 0);              // turn off servo
    if ( p_direction[i]== p_straight[i])
      {
      ...
      {
        bld[i]->setPWM(l_diverging[i], 4096, 0);
        bls[i]->setPWM(l_straight[i], 0, 0);
      }
  }
